...
// Truy vấn dữ liệu
$sql    = "SELECT san_pham_id, hinh_anh, gia_san_pham_mh, ten_san_pham FROM bo_suu_tap_mh ORDER BY san_pham_id";
$result = $conn->query($sql);
?>
<!DOCTYPE html>
<html lang="vi">
...
<body>
    ...
    <!-- Nội dung chính -->
    <main>
        <h1 style="text-align:center; padding-top:20px;">Bộ Sưu Tập Mùa Hè</h1>
        <section class="product-section">
            <?php if ($result && $result->num_rows > 0): ?>
                <?php while ($row = $result->fetch_assoc()): ?>
                    <?php 
                        // Sửa đường dẫn ảnh cho đúng dấu /
                        $img = $row['hinh_anh'] ? str_replace('\\', '/', $row['hinh_anh']) : 'imagin/default.jpg';
                        $gia = number_format($row['gia_san_pham_mh'], 0, ',', '.');
                    ?>
